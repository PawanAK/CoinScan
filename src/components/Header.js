...
const Header = () => {
  ...
  const changeToFVM = async () => {
    await ethereum.request({method: "wallet_switchEthereumChain", params: [{chainId: "3141"}]})
    setShowChains(false)
  }
  ...
  const changeToGoerli = async () => {
    await ethereum.request({method: "wallet_switchEthereumChain", params: [{chainId: "5"}]})
    setShowChains(false)
  }
  ...
  return (
    <div className="w-full h-1/4 pt-4 flex justify-between items-start">
      {/* Logo */}
      <img className="h-12 ml-2" src="w3pay.png" />
      ...
      <div className="flex justify-between items-start">
        ...
        <div onClick={() => setShowChains(true)} className="text-xl py-2 px-4 mr-2 font-sans border-opacity-60 border-2 border-blue-900 font-medium cursor-pointer bg-black text-white rounded-lg flex justify-between items-center">
          {App.chain == "FVM" || App.chain == "Goerli" ? (
            // <img className="h-6 mr-2" src="ethereum-eth.svg" />
            // <img className="h-6 mr-2" src="../fvmimg.png" />
            <img className="h-6 mr-2" src="https://user-images.githubusercontent.com/310223/150189748-ed673f1a-6a5f-4746-a1b3-0d8b1a3f5fd2.png" />
          ) : (
        ...
        <div className={`${showChains ? "" : "hidden"} absolute right-0 z-50`}>
          {/* FVM */}
          <div onClick={changeToFVM} className="text-xl py-2 px-4 mr-2 font-sans border-opacity-60 border-2 border-blue-900 font-medium cursor-pointer hover:bg-gray-900 bg-black text-white rounded-lg flex justify-between items-center">
            <img className="h-6 mr-2" src="./fvmimg.png" />
